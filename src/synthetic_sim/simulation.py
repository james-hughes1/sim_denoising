...
from .otf import calc_psf, PsfParameters


def arange_zero(n, spacing=1):
    """!
    @brief Returns an array A with A[n//2] = 0.0 and A[m] - A[m-1] = spacing.

    @param n (int) - Length of array
    @param spacing (int, optional) - Value of A[m] - A[m-1]. Default: 1
...
def threshold_norm(sample):
    """!
    @brief Applies a threshold and normalises the sample to improve contrast.

    @param sample (np.ndarray) - Sample volume

    @returns np.ndarray volume with transformed pixel values
    """
...
class Simulator:
    """!
    @brief The Simulator class encapsulates the state of a 3D microscope
    ...
    def __init__(self, **kwargs):
        """!
        @brief Initialises constant parameters.
        """
    ...
    def randomise(self):
        """!
        @brief Initialises random parameters.
        """
    ...
    def params_dict(self):
        """!
        @brief Returns optical system parameters.
    ...
    def psf_params(self):
        """!
        @brief Returns a PsfParameters object for generating an appropriate
        PSF.

        @returns PsfParameters
        """
    ...
    def wavevectors(self):
        """!
        @brief Calculates wavevectors inside the sample for the three beams,
    ...
    def illumination(self):
        """!
        @brief Calculates the illumination intensity in the sample.

        @returns ndarray of shape (n_rotations, n_shifts, n_x, n_x, n_z)
        """
    ...
    def in_focus_plane(self, sample):
        """!
        @brief Returns the designated `ground truth' plane.
    ...
    def psf(self):
        """!
        @brief Calculates a PSF if it has not been done already.
    ...
    def simulate_sim(self, sample):
        """!
        @brief Calculates the 15 simulated SIM images for a given sample.

        @param sample np.ndarray

        @returns np.ndarray SIM acquisition stack in A,P,X,Y format
        """
        psf = self.psf()
    ...
    def simulate_ideal_superres(self, sample):
        """!
        @brief Simulates the best-case scenario for a 3D SIM reconstruction, by
    ...
    def add_noise(self, image):
        """!
        @brief Adds a combination of Gaussian and Poissonian noise to the
        image.

        @param image (np.ndarray) - SIM acquisition image

        @returns np.ndarray image with added noise
        """
        rng = np.random.default_rng(seed=10052024)
...
class SimulationRunner:
    """!
    @brief Class which performs a batch of simulations, either sequentially or
    in parallel.
    """
    ...
    def __init__(self, input_dir, output_dir, index_range, z_offset):
        ...
        @param input_dir (str) - Directory of images volumes
        @param output_dir (str) - Directory for saving synthetic SIM volumes
        @param index_range (range) - Determines which indices of sorted file
        list to process
        @param z_offset (int) - Determines which axial planes to use.
        0 corresponds to the top of the image volume
        """
    ...
    def do_sim(self, i, sim, vol):
        """!
        @brief Creates a new random virtual microscope simulator, takes a new
        ...
        @param i (int) - Index of simulation. Used for labelling output files
        @param sim (Simulator) - Object storing parameters of optical system
        and simulation methods
        @param vol (np.ndarray) - Image volume
        """
    ...
    def run(self):
        """!
        @brief Runs a series of simulations sequentially.
        """
